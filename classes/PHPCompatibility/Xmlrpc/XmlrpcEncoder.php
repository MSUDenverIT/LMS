 ...
 * Note: Fixed encodeCall() to handle associative arrays.
 */
...
class XmlrpcEncoder {
	 ...
	 * Constructor method
	 */
	final public function __construct( $encoding = '' ) {
	...
	final public function setValueType(&$value, $type) {
		...
		if ( empty($value) || !in_array(strtolower($type), array("base64", "datetime", "cdata")) ) throw new Exception("Invalid value type");
	...
	private function encodeValue(XMLWriter $xml, $value) {
		...
		} else throw new Exception("Unknown type for encoding");
	}
